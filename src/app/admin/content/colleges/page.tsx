...
async function deleteCollege(formData: FormData): Promise<void> {
    "use server";
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-8 border-b-4 border-black pb-4">
                ...
                <Link href="/admin/content/colleges/new" className="brutal-btn bg-[#4ade80] px-6 py-3 font-black uppercase text-black">+ Add College</Link>
            </div>
            <div className="bg-white border-4 border-black brutal-shadow-sm overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                        {colleges.length === 0 ? (
                            <tr><td colSpan={5} className="p-8 text-center font-bold text-xl uppercase">No colleges yet.</td></tr>
                        ) : (
                            colleges.map((college) => (
                                <tr key={college.id} className="border-b-4 border-black hover:bg-[var(--color-bg)] transition-colors">
                                    <td className="p-4 border-r-4 border-black font-bold uppercase">{college.name}</td>
                                    <td className="p-4 border-r-4 border-black font-bold text-sm">{college.location || `${college.city || ''}, ${college.state || ''}`}</td>
                                    <td className="p-4 border-r-4 border-black">
                                        <span className="brutal-badge border-black bg-[var(--color-primary-yellow)]">{college.type || 'University'}</span>
                                    </td>
                                    ...
                                    <td className="p-4 flex gap-2">
                                        <Link href={`/explore/colleges/${college.slug}`} className="px-3 py-1 border-2 border-black bg-white font-bold text-sm brutal-shadow-sm hover:translate-x-[2px] hover:translate-y-[2px] transition-all" target="_blank">VIEW</Link>
                                        <DeleteButton id={college.id} name={college.name} deleteAction={deleteCollege} />
                                    </td>
                                </tr>
                            ))
                        )}
