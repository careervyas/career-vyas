...
async function deleteExam(formData: FormData): Promise<void> {
    "use server";
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-8 border-b-4 border-black pb-4">
                ...
                <Link href="/admin/content/exams/new" className="brutal-btn bg-[#f43f5e] px-6 py-3 font-black uppercase text-white">+ Add Exam</Link>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-black text-white uppercase text-sm font-black border-b-4 border-black">
                            ...
                            <th className="p-4 border-r-4 border-black">Mode</th>
                    ...
                    <tbody>
                        {exams.length === 0 ? (
                            <tr><td colSpan={5} className="p-8 text-center font-bold text-xl uppercase">No exams yet.</td></tr>
                        ) : (
                            exams.map((exam) => (
                                <tr key={exam.id} className="border-b-4 border-black hover:bg-[var(--color-bg)] transition-colors">
                                    ...
                                    <td className="p-4 border-r-4 border-black"><span className="brutal-badge border-black bg-[var(--color-bg)]">{exam.level || 'National'}</span></td>
                                    <td className="p-4 border-r-4 border-black font-bold text-sm">{exam.mode || 'N/A'}</td>
                                    ...
                                    <td className="p-4 flex gap-2">
                                        <Link href={`/explore/exams/${exam.slug}`} className="px-3 py-1 border-2 border-black bg-white font-bold text-sm brutal-shadow-sm hover:translate-x-[2px] hover:translate-y-[2px] transition-all" target="_blank">VIEW</Link>
                                        <DeleteButton id={exam.id} name={exam.name} deleteAction={deleteExam} />
                                    </td>
                                </tr>
                            ))
                        )}
